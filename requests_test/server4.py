...
#        if s.path == '/validate':
        if s.path.find('validate') > 0:
            ...
            s.end_headers()
            s.wfile.write(r.text)
            #s.wfile.write(user + " has rights level " + result)
            ...
            if int(result) > 0:
                s.wfile.write('{"result":"already registered"}')
                ...
                if int(result) == 1:
                    s.wfile.write('{"result":"register successful"}')
